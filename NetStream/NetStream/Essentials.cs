...
namespace NetStream
{
    public class Essentials
    {
        ...
        public static async void GetSelectedIndexers()
        {
            try
            {
                if (PlatformDetector.GetPlatform() == Platform.Web || PlatformDetector.GetPlatform() == Platform.Android)
                {
                    var allIndexers = await JackettService.GetIndexersAsync();
                    if (allIndexers == null || !allIndexers.Any())
                    {
                        Log.Information("No indexers available from Jackett service");
                        return;
                    }
                ...
                {
                    if (File.Exists(AppSettingsManager.appSettings.IndexersPath))
                    {
                        var text = File.ReadAllText(AppSettingsManager.appSettings.IndexersPath);
                        if (String.IsNullOrWhiteSpace(text))
                        {
                            ...
                            File.WriteAllText(AppSettingsManager.appSettings.IndexersPath, js);
                        }
                        else
                        {
                            bool changed = false;
            ...
            {
                Log.Information($"Error in GetSelectedIndexers: {ex.Message}");
            }
        }
        ...
        //     catch (Exception e)
        //     {
        //         Console.WriteLine("checkInstalled Error: ");
        ...
        public static string FindExeManually(string exeName)
        {
            try
            {
                var searchPaths = new[]
                ...
                foreach (var basePath in searchPaths)
                {
                    var exePath = Directory
                        .EnumerateFiles(basePath, exeName, SearchOption.AllDirectories)
                        .FirstOrDefault();
        ...
        public static void InstallJackett()
        {
            try
            {
                ...
                AppSettingsManager.appSettings.JacketApiUrl = !string.IsNullOrWhiteSpace(jacketApiUrl)
                    ? jacketApiUrl
                    : "http://127.0.0.1:9117/";
            ...
            {
                Log.Information($"Error installing Jackett: {e.Message}");
            }
        }
        ...
        public static void InstallSilentlyJackett()
        {
            ...
            {
                Log.Information("Error during installation: " + e.Message);
            }
        }
        ...
        public static bool StartJackettService()
        {
            ...
            {
                Log.Information($"Error starting Jackett service: {ex.Message}");
            }
